...
class FasterWhisperASR(ASRBase):
    ...
    def load_model(self, modelsize=None, cache_dir=None, model_dir=None):
        ...
        # Create the model with the specified device
        model = WhisperModel(modelsize, device=device, compute_type="float16")
        return model
...
### 实际使用的
class InsanelyFastWhisperASR(ASRBase):
    """Uses the Insanely Fast Whisper pipeline for ASR."""

    sep = " "  # 设置单词之间的分隔符

    def load_model(self, modelsize="openai/whisper-large-v2", cache_dir=None, model_dir=None):
        # 设置模型和设备
    ...
    def transcribe(self, audio_data, prompt=None, *args, **kwargs):
        # 将音频数据传递给 pipeline 进行转录
        outputs = self.pipeline(
            audio_data,  # 传递音频数据
            chunk_length_s=30,  # 每块处理的长度
            batch_size=24,  # 批次大小
            return_timestamps=True,  # 返回时间戳
        )
        print("Transcription result:", outputs)
        return outputs
    ...
    def segments_end_ts(self, result):
        if 'chunks' in result:
            return [word_info['timestamp'][1] for word_info in result['chunks']]
        return []
    ...
    def set_translate_task(self):
        # 设置为翻译任务（如果需要）
        self.pipeline.model.config.task = "translate"
...
class HypothesisBuffer:
    ...
    def insert(self, new, offset):
        try:
            new = [(a + offset if a is not None else offset, b + offset if b is not None else offset, t) for a, b, t in new]
            ...
            logger.warning("TypeError: unsupported operand type(s) for +: 'NoneType' and 'float'")
...
class OnlineASRProcessor:
    ...
    def to_flush(self, sents, sep=None, offset=0, ):
        ...
        if len(sents) == 0:
            ...
            b = offset + sents[0][0] if sents[0][0] is not None else offset
            e = offset + sents[-1][1] if sents[-1][1] is not None else offset
        return (b, e, t)
...
def add_shared_args(parser):
    ...
    parser.add_argument('--lan', '--language', type=str, default='zh',
                        help="Source language code, e.g. en,de,cs, or 'auto' for language detection.")
    parser.add_argument('--task', type=str, default='transcribe', choices=["transcribe", "translate"],
                        help="Transcribe or translate.")
    parser.add_argument('--backend', type=str, default="faster-whisper",
                        choices=["faster-whisper", "whisper_timestamped", "brain-api"],
...
def asr_factory(args, logfile=sys.stderr):
    ...
    if backend == "brain-api":
        logger.debug("Using OpenAI API.")
        asr = OpenaiApiASR(lan=args.lan)
    else:
        if backend == "faster-whisper":
            # asr_cls = FasterWhisperASR
            asr_cls = InsanelyFastWhisperASR
        else:
    ...
    if args.task == "translate":
        ...
        tgt_language = "zh"  # Whisper translates into English
    else:
